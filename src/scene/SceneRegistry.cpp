...
struct Registry {
    std::unordered_map<std::string, InkingScene*> byName;
    std::vector<InkingScene*> order;  ///< 登记顺序，Latest() 取最后一个
};
...
void SceneRegistry::Register(InkingScene& scene) {
    Registry& r = registry();
    r.byName[scene.GetName()] = &scene;  // 重名时后来的覆盖先来的
    r.order.push_back(&scene);
}

void SceneRegistry::Unregister(InkingScene& scene) {
    Registry& r = registry();

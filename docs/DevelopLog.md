...
> 基于文档做一个按钮和一个场景：**场景纯白，按钮半透明黑**；悬停时
> 黑色加重，同时**稍微变大**——专门把「变大会导致重新烘焙」这条路走通。
>
> - `Button::SetHoverGrowth(factor)`：悬停时按倍率放大，走的是
>   `Resize()` 这个 `[final]` 写入口，所以命中表与重绘的标脏都在框架内部完成，
>   按钮自己不碰脏标记；离开悬停精确还原原尺寸（不累积取整误差）。
>   锚点用「自身 Center 对上父级 Center」时是从中心长大，中心点不动。
> - 同一个悬停动作里两条路并存，正好互为对照：
>   黑色加重 = 只改颜色 → `makeDirty()`（不重烘）；
>   变大 = 改几何 → 标脏 → 上层命中表重烘一次。
> - 渲染器开了 `SDL_BLENDMODE_BLEND`：SDL 的绘制默认不混合，不开的话
>   半透明色会被当成不透明直接画上去。
>
> 验证方式（三层，都过了）：
>
> 1. 自检 `tests/test_main.cpp` 第 14 节：悬停 1.2 倍放大后，表里的条目矩形
>    就是放大后的矩形（从中心四周各外扩 20），新扩出来那一圈能命中、缩回后
>    又不命中，兄弟按钮不受影响，重烘次数正好 +1 / +1；对照组（只改颜色、
>    没设倍率）一次都不重烘。
> 2. 真实事件链路：临时程序推一个 `SDL_EVENT_MOUSE_MOTION` 进队列，走
>    `MouseInput → InputRouter → 悬停 → 放大 240×120 → 276×138 → 重烘`，
>    日志里能看到「尺寸变成 276×138 → 命中表要重烘」和「面板表重建了一次」。
> 3. 取色（1920×1080 全尺寸帧）：场景纯白 `#FFFFFF`；按钮常态
>    `#878787`（黑 α=120 压在白上），悬停 `#373737`（α=200，黑更重）；
>    原本按钮右缘外 10px 的那点在悬停帧里也变成 `#373737`——变大是真的变大，
>    不是只改了颜色。
>
> 示例：`examples/hover_grow`（纯白场景 + 3 颗半透明黑按钮，点击只写日志）。
